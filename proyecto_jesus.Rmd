...
```{r}
library(readxl)
excel_info <- read_excel("data/Personas detallado_WEB.xls", sheet = "Hoja1", range = "C7:F148", col_names = FALSE)
...
df <- df %>% mutate_all(~ ifelse(str_detect(string = ., pattern =  " "), NA, .)) # con esto cojo cuando hay uno o más espacios; cambio si detecta uno a NA
```
