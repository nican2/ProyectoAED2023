...
library(pacman)
pacman::p_load(readr, stringr, tidyr, dplyr, readxl, ggplot2, forcats, haven, kableExtra, VIM, e1071, ggstatsplot, gridExtra, reshape2, editrules, mice, colorspace, leaflet, ggmap, ggmosaic, magick, GGally)
```
...
A diferencia de las variables numérica, en las variables categóricas no es posible obtener muchos estadísticos. El más común es la moda, que se corresponde con la columna *topCount* de la Tabla \ref{tab:kable_sumario}.
...
df_test <- df %>%
  ...
  summarise(MediaHijos = mean(NHIJOS, na.rm = TRUE), MediaSatisfaccion = mean(SATISTIEMP, na.rm = TRUE)) %>%
  mutate(punto_medio = puntos_medios)

df_test <- na.omit(df_test)
head(df_test, 10)
```
```{comment, grupos_edad_comment}
Es notable apreciar que la media de hijos se incrementa según avanzamos en la variable grupo_edades, como cabría esperar. La media de satisfacción por otra parte parece seguir una ascensión conforme se incrementa la edad. Sin embargo, en el intervalo de "77-86" hay una caída algo inesperada. 
```
...
```{r campo_by_sexo, echo = FALSE, include = TRUE, fig.width = 5, fig.height = 3, fig.align = "center", fig.cap = "Gráfico de barras del campo de estuido en función del sexo\\label{fig:campo_by_sexo}", fig.pos = "H"}
df_plot <- df
...
La correlación de Pearson es de gran utilidad para poder determinar la dependencia lineal entre diferentes variables. Alizando los resultados obtenidos, sólo se puede apreciar una dependencia entre los pares de variables EDAD y ANOESTUD, y, ANOESTUD y EDADESTUD. Entre el resto de variables numéricas no existe una dependencia.

Para evaluar la relación entre variables numéricas es mediante la correlación de Spearman. En este caso, es un estadístico que nos da la relación monótona entre pares de variables, es decir, no mide únicamente la relación lineal entre las variables.
...
Como se puede observar, en el gráfico de la Figura \ref{fig:corr_plot_spearman}, no parecen existir relaciones entre variables numéricas desconocidas hasta el momento. Pero es importante comprobar las correlaciones de Spearman ya que puede darse el caso de que variables que no presentan una relación lineal entre si, sí presente una relación no lineal.

#### Variables categóricas

En el caso de las variables categóricas, hay diferentes formas de analizar la relación entre las variables. Se pueden estudiar variables a pares mediante la realización del test de chi-cuadrado. En otros casos, también es común el empleo de la V de Cramer, que consite en la raíz cuadrada del valor normalizado del test de chi-cuadrado. Como alternativa a estos estadísticos, se puede emplear la tau de Goodman y Kruskal.

En el caso de los contrastes de hipótesis (chi-cuadrado o V de Cramer), sirven para evaluar la dependencia o independencia de variables categóricas. Mediante el uso de *assocstats* de la librería *vcd*, con la cual se obtiene el valor de la V de Cramer se ha tratado de analizar la realación entre la variable ESTUDIOS y LUGTRAB.

```{r v_cramer, include=FALSE, eval=TRUE}
library(vcd)
tab <- table(df$ESTUDIOS, df$LUGTRAB)
assocstats(tab)
```
Como se puede observar en los resultados obtenidos, el resulado de la V de Cramer para ese par de variables es 0.085. Teniendo una significancia de 0.05, se ha obtenido que 0.085 > 0.05, por lo tanto se aceptaria la hipótesis nula. Es decir, las variables son independientes (H0).


En el caso de la tau de Goodman y Kruskal, el valor de tau es asimetético (hecho que no sucede con otros estadísticos como es la correlación) esto es debido a que dicho estadístico se basa en la fracción de variabilidad de la categoría y que puede ser explicada por la variable categórica x. Teniendo en cuenta lo anterior, se ha tratado de representar un mapa con este estadístico para las variables categóricas.

```{r goodman_kruskal, echo = FALSE, include = TRUE, fig.width = 6, fig.height = 5, fig.align = "center", fig.cap = "Mapa de GoodManKruskal para las variables categóricas\\label{fig:goodman_kruskal}", fig.pos = "H"}
...
+ El nivel de estudios de una persona es influyente en el campo de trabajo (0.4). En cambio, el campo de trabajo no es tan influyente en los estudios de una persona (0.22).

+ El estado civil de una persona es muy influyente en si tendrá o no hijos (0.56).

+ La situación laboral de una persona es influyente en su lugar de trabajo (0.48). En cambio, el lugar de trabajo de una persona, no es tán influyente en su situación laboral.
...
Para la primera aproximación, los resultados son los siguientes:
```{r clustering1,echo=FALSE, include=FALSE}
...
#Se defiune un numero de clusters, que debera ser ajustado para 
...
En cuanto al tratamiento con variables categóricas, el tratamiento es parecido, salvo que en esta ocasión es necesario aplicar one-hot encoding a las variables para transformarlas en numéricas, al ser requerido por el algoritmo de clustering.


```{r clustering2 ,echo=FALSE, include=FALSE}
...
```{r kable_cl2, include = T}
kable_res <- knitr::kable(SumarioDatos(data = df_copia), format = "latex",
             booktabs = TRUE, 
...
kable_styling(kable_res, font_size = 5)
```
