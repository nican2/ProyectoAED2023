...
Primeramente, se cargan todas las librerías necesarias en las diferentes fases del proyecto. Esto se hace de manera más elegante utilizando el paquete `pacman` nuestro lenguaje de programación, R.

```{r librerias, message = F, include = T, echo = F}
# Carga de librerías necesarias con pacman
library(pacman)
pacman::p_load(readr, stringr, tidyr, dplyr, readxl, ggplot2, forcats, haven, kableExtra, VIM, e1071, ggstatsplot, gridExtra, reshape2, editrules, mice, colorspace, leaflet, ggmap, ggmosaic)
```
...
En resumen, nuestro dataset cuenta con `r nrow(df)` observaciones y con `r ncol(df)` variables. Muchas de estas variables no nos serán de utilidad para las preguntas que deseamos responder, por lo que convendrá deshacernos de ellas por una cuestión de comodidad y optimización
...
Tras aplicar estas reglas a nuestro conjunto de datos, obtenemos que no hay ninguna violación de estas (obtenemos un NULL), por lo que no será necesario hacer una imputación o eliminado de dichas observaciones.
...
df_test <- df %>%
  ...
  summarise(MediaHijos = mean(NHIJOS, na.rm = TRUE)) %>%
...
ggplot(data = df_aux) +
  ...
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x = "Tareas domésticas", y = "Situación laboral")

```

Un gráfico de mosaico es una herramienta de visualización muy poderosa. Podemos visualizar 3 variables de manera simultánea en la Figura \ref{fig:mosaico}. Pueden apreciarse algunos comportamientos esperados, como que las personas que tienen
