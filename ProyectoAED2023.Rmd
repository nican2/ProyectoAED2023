...
*dr_ECEPOVhogar_2021.xlsx*, se ha considerado que muchas de las variables no nos otorgarán interés para el análisis que particularmente se desea aplicar, por lo que se seleccionarán aquellas variables consideradas más relevantes.
...
<!-- Númerica - Númerica -->
Se podría hacer un gráfico de dispersión per no sale nada vistos
<!-- Por usar uno se podria usar el siguiente (no hay muchas mas variables) -->
...
ggplot(data=df_sin_na, aes(x = EDAD, y = NHIJOS, color = SATISTIEMP)) +
  geom_jitter(size = 2, alpha=0.5) +  # Añade jittering
...
```
En el podemos observar información acerca del dataframe como el hecho de que no existen una gran cantidad de instancias en nuestro conjunto de datos tales que para para edades elevadas contengan valores acerca de su satisfacción acerca de las compras online, a pesar de haber instancias con edades elevadas.  Asimismo, se puede pbservar que un número elevado de hijos podría estar asociado con una probabilidad mayor de que la satisfacción sea calificada como muy negativa.
...
Para la relación entre variables categoricas, se decide usar un diagrama de mosaico
```{r}
library(ggmosaic)

df_sin_na <- df[complete.cases(df$CAMPO, df$SITLAB), ]

ggplot(data=df_sin_na) +
  geom_mosaic(aes(x = product(CAMPO, SITLAB))) +
  labs(title = "Mosaic Plot", x = "Variable 1", y = "Variable 2")

library(stringr)
wrapped_labels <- str_wrap(levels(df_sin_na$CAMPO), width = 10)  # Adjust width as needed
...
wrapped_labels_CAMPO <- str_wrap(levels(df_sin_na$CAMPO), width = 10)
wrapped_labels_SITLAB <- str_wrap(levels(df_sin_na$SITLAB), width = 10)

ggplot(data = df_sin_na, aes(x = CAMPO, y = SITLAB)) +
  geom_tile(stat = "identity") +
  labs(title = "Mosaic Plot", x = "Variable 1", y = "Variable 2") +
  scale_x_discrete(labels = wrapped_labels_CAMPO) +
  scale_y_discrete(labels = wrapped_labels_SITLAB) +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) 
#sapply(df,function(x){ifelse(is.factor(x),length(unique(x)),0)})


df_sin_na$combined <- interaction(df_sin_na$CAMPO, df_sin_na$SITLAB)

# Create wrapped versions of the labels
wrapped_labels_combined <- str_wrap(levels(df_sin_na$combined), width = 10)

ggplot(data = df_sin_na, aes(x = "", y = combined, fill = ..count..)) +
  geom_mosaic() +
  labs(title = "Mosaic Plot", x = "Variable 1", y = "Variable 2") +
  scale_y_discrete(labels = wrapped_labels_combined)


library(vcd)
df_plot <-df[, c("CAMPO", "SITLAB", "EDAD")]

df_plot_tab <- xtabs(EDAD ~ CAMPO + SITLAB, data=df_plot)

largs <- list(set_varnames=list(CAMPO="Campo", 
                                SITLAB="Situacion"),
              abbreviate=10)
...
# Configurar parámetros para el gráfico
largs <- list(set_varnames=list(CAMPO="Campo", 
                                SITLAB="Situacion"),
              abbreviate=10)  # Ajustar el número para acortar más o menos los niveles
gargs <- list(interpolate=c(1,2,4,8))
par(mfrow=c(1,1), mar=c(5,5,20,20))
...
par(mfrow=c(1,1),  mar=c(8,8,4,2), oma=c(4,4,0,0))
...
# Aumentar el espacio entre las etiquetas del eje y los valores
mosaic(df_plot_tab, 
       shade=TRUE, 
       labeling_args=largs, 
       gp_args=gargs,
       main="Relacion campo-siutación laboral-edad", 
       legend=FALSE, 
       rot_labels=c(85,90,0,0),
       
       )  # Ajustar los valores según sea necesario



mosaicplot(table(df$EC, df$SITLAB, df$SEXO),
...
```{r}
ggplot(df, aes(x = CAMPO, fill = SEXO)) +
  geom_bar(position = "stack") +
  labs(title = "Relaciones entre el campo y el sexo",
       x = "Campo",
       y = "Total") +
...
ggplot(df, aes(x = CAMPO, fill = SEXO)) +
  ...
  labs(title = "Gráfico de Barras por Sexo y Campo", x = "Campo", y = "Frecuencia") +
  scale_fill_manual(values = c("Hombre" = "blue", "Mujer" = "pink"), na.value = "grey")  # Puedes personalizar los colores según tus preferencias


etiquetas_personalizadas <- c("etiqueta1", "etiqueta2", "etiqueta3", ...)
...
ggplot(df_plot, aes(x = CAMPO, fill = SEXO)) +
  geom_bar(position = "dodge", stat = "count") +
  labs(title = "Gráfico de Barras que relaciona Sexo y Campo de estudio", x = "Campo", y = "Frecuencia") +
...
ggplot(df_cl, aes(x=EDAD, y=SATISTIEMP,alpha=ANOESTUD, color = clust)) +
  geom_jitter() +
  labs(title = "Valores atipicos detectados",
       x = "Edad",
       y = "Satisfaccion compras online",
       alpha="Año estudio",
       color = "Cluster")+
  theme_classic()
...
```
  ...
  <!-- Aquí podríamos poner una gráfica de barras en las que cada barra la pinta de un color en función de otra varible categórica -->

## Análisis multivariante (todo lo que sea más de tres variables)
