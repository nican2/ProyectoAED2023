...
if(resultado_chi_cuadrado$p.value < 0.05) {
  print("Se rechaza la hipótesis nula de independencia entre las variables con un nivel de significancia de 0.05. Hay cierta asociación entre ellas.")
}
```
$\\$
...
Por un lado, mediante la realización de este proyecto ha sigo posible a sigo posible obtener las respuestas a las diferentes preguntas planteadas al inicio del proyecto. En primer lugar hemos visto, mediante el análisis univariante, cómo están distribuidas las edades de las personas encuestadas y su situación laboral. En segundo lugar, mediante el análisis multivariante, ha sido posible examinar las relaciones entre diferente grupos de variables. Se ha analizado la relación entre la edad y el número de hijos. O, entre la edad y su grado de satisfacción con el desplazamiento al trabajo. Incluso, se ha implementado un mapa de España interactivo que permite analizar el grado de satisfacción de las personas en función de su lugar de residencia, donde se ha comprobado que en las dos grandes ciudades del país es donde se encuentran las personas más descontentas con su tiempo de desplazamiento hasta su puesto de trabajo. Para finalizar con el análisis multivariante, se ha generado un diagrama de barras y un mosaico con los que se ha podido entender la relación entre variables no continuas como el sexo y el campo de estudio de las personas o su grado de aportación el las tareas del hogar. Seguidamente, se ha procedido con la caracterización tanto de variables tanto numéricas como categóricas. Donde hemos comprobado que variables contínuas como la edad o el año de finalización de los estudios están linealmente realcionadas. O, que la situación laboral de una persona está muy ligada a su lugar de trabajo. Y, por último, también se han implementado diferentes métodos para detectar los outlier o datos atípicos en variables individuales y, mediante el uso del clustering, en la cobinación de varias variables numéricas y en la combinación de variable numéricas y categóricas.

Por el otro lado, gracias a la realización de este proyecto ha sido posible poner en práctica todos los conceptos aprendidos en la asignatura de Análisis Exploratorio de Datos del máster en Ciencia de Datos. Hemos podido enfrentarnos a un problema real. Trabajando con un conjunto de datos real. Conyevando esto el enfrentamiento a diferentes fases como han sido: la carga y lectura del dataset, la realización de las modificaciones pertinentes en el dataset para facilitar su manejo y estudio, el análisis de las diferentes variables o la detección de valores atípicos contenidos en dicho dataset.
