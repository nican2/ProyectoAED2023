...
Para el caso de las variables numéricas, una técnica común es la creación de un mapa de correlaciones con todas las variables del dataset. En nuestro caso, se corresponde con el mostrado en la Figura \ref{fig:corr_plot}.
...
corr_mat <- round(cor(as.data.frame(df_numericas_sin_nas)), 2)
```
...
```{r corr_plot, echo = FALSE, include = TRUE, fig.width = 4, fig.height = 3, fig.align = "center", fig.cap = "Mapa de correlaciones para las variable numéricas\\label{fig:corr_plot}", fig.pos = "H"}
...
Analizando los resultados obtenidos, sólo se puede apreciar una dependencia entre los pares de variables EDAD y ANOESTUD, y, ANOESTUD y EDADESTUD. Entre el resto de variables numéricas no existe una dependencia.


```{r ggcorr_plot, eval=FALSE}
ggpairs(df_numericas_sin_nas)
```
