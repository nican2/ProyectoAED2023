...
library(knitr)
options(width = 100)
knitr::opts_chunk$set(echo = F, message = F, error = F, warning = F, comment = NA, dpi = 80, tidy = F, cache.path = '.cache/', fig.path = './figure/', include = F, fig.width = 5.0, fig.height = 4, fig.align = "left")
```
...
var_interes <- c("IDQ_PV", "TAM_MUNI", "SEXO", "EDAD", "NACIM", "PNACIM", "EC", "EDADEC", "ESTUDIOS", "ANOESTUD", "EDADESTUD", "CAMPO", "SITLAB", "FLEXI", "LUGTRAB", "SATISTIEMP", "COMPRAINT", "NHIJOS", "HIJOS", "TDOMEST")
...
```{r sitlab_bar, echo = FALSE, include = TRUE, fig.width = 4, fig.height = 3, fig.align = "center", fig.cap = "Q-Q plot para la variable EDAD."}
...
detectar_outliers <- function(data, methods = c("tresSigma")) {
  ...
  if ("Hampel" %in% methods) {
    # Método Hampel
    MADM <- 1.4826 * median(abs(data - median(data))) 
...
results <- detectar_outliers(na.omit(df$SATISTIEMP), methods = methods_to_apply)
...
```{r seleccion_una_variable}
x <- "NHIJOS"

df_var <- df_sum_outliers %>%
  select(all_of(x), starts_with(paste0(x, "_Outliers"))) %>%
  rename_at(vars(starts_with(paste0(x, "_Outliers"))), ~str_remove(., paste0(x, "_Outliers_"))) %>%
  pivot_longer(cols = -all_of(x), names_to = "Method", values_to = "Outlier_Status")

ggplot(df_var, aes(x = Method, y = NHIJOS, color = Outlier_Status)) +
  geom_point() +
  scale_color_manual(values = c("TRUE" = "red", "FALSE" = "blue")) +
  labs(
    title = "Distribución de NHIJOS con Detección de Outliers",
...
```{r}
outlier_values <- unique(boxplot.stats(df$NHIJOS)$out)  # outlier values.
boxplot(df$NHIJOS, main="Distancia", boxwex = 0.1)
...
#### Analisis entre dos variables numericas
Un ejemplo de este tipo de analisis es el siguiente
```{r}
...
#### Analisis de una variable numérica frente a una categórica
```{r}
df_plot <- df
