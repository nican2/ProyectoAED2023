...
library(pacman)
pacman::p_load(readr, stringr, tidyr, dplyr, readxl, ggplot2, forcats, haven, kableExtra, VIM, e1071, ggstatsplot, gridExtra)
```
...
A pesar de que fue de gran utilidad el fichero que ofrecía INE para la transformación de las variables a un tipo correcto, aparecieron algunos fallos que tuvimos que solventar de manera manual. Determinadas variables no fueron bien codificadas por parte del INE, de tal forma que hubo que modificar ligeramente el fichero .xlsx. Concretamente, fue en algunos *diccionarios* de variables en los que no terminaba de estar correctamente determinado el *código* (lo que sería el level en un factor). El error presentado (en algunas variables) consistía, básicamente, en que aparecía "01" en lugar de "1" (para otros números similar), valor recogido en las observaciones.
...
Las variables SEXO, NACIM y PNACIM de este nuevo dataset deben ser convertidas a sus tipos correctos, por lo que se hace uso de la información que proporciona el INE sobre las variables.
...
Podemos hacernos una idea rápida de cuáles son los datos pertenecientes a nuestro dataframe, *df*, haciendo uso de la función glimpse, perteneciente a la librería dplyr. 
...
*dr_ECEPOVhogar_2021.xlsx*, consideramos que muchas de las variables no nos otorgarán interés para el análisis que particularmente nosotros deseamos aplicar, por lo que seleccionaremos aquellas que hemos considerado más relevantes.
...
Otra variable numérica sujeta a estudio en este caso será el año de estudio
...
ggplot(frecuencias, aes(x = reorder(Situacion, -n), y = n, fill = n)) +
  geom_bar(stat = "identity",
           col = "black") +
  labs(title = "Diagrama de Barras para representar la Situación Laboral",
       x = "Categoría",
       y = "Frecuencia") +
...
Pasamos a generar gráficos para visualizar la relación entre diferentes variables.
...
```{r crosstable, include=TRUE}
library(gmodels)
kable_crosstab <- knitr::kable(as.data.frame(CrossTable(df$ESTUDIOS, df$SITLAB)), format = "latex",
             booktabs = TRUE, 
             caption = "Tabla cruzada para comparar los estudios realizado con la situación laboral", 
             align = 'ccc', centering = FALSE,
             table.envir = "table",
             tabular.environment = "tabularx")
```
...
```{r mosaico_categoricas}
ggplot(df, aes(x = Genero, y = Musica, fill = Edad))+
  geom_tile() +
  labs(x = "Género", y = "Música", fill = "Edad") +
  theme_minimal()
...
# Puedes visualizar el dendrograma resultante
...
ggplot(df, aes(x = EC, y = EDADEC, fill = is.na(EDADEC))) +
  geom_boxplot() +
  scale_fill_manual(values = c("TRUE" = "red", "FALSE" = "blue")) +
  labs(title = "Distribución de Variable Continua por Factor y NA",
       x = "Factor",
       y = "Variable Continua",
       fill = "NA") +
  theme_minimal()
...
ggplot(df, aes(x = EC, y = EDADEC)) +
  geom_boxplot(aes(fill = is.na(EDADEC))) +
  geom_point(data = df[is.na(df$EDADEC), ], aes(x = EC, y = EDADEC), color = "red", shape = 19) +
  scale_fill_manual(values = c("TRUE" = "blue", "FALSE" = "lightgray")) +
  labs(title = "Distribución de Variable Continua por Factor con NA",
       x = "Factor",
       y = "Variable Continua",
       fill = "NA") +
  theme_minimal()
...
chi2 <- chisq.test(table(is.na(df$EC), df$EDADEC))
chi2
```

```{r}
# Funcion que estudia la correlacion de NA de una columna frente a los valores de otra columna
correlacion_cols <- function(c1,c2){
  valores_na <- is.na(c1)
  columna_a_con_na <- ifelse(valores_na, 1, 0)
  correlacion <- cor(columna_a_con_na, columna_b)
  return(correlacion)
}

col_factor <- df[sapply(df, is.factor)]
col_numeric <- df[sapply(df, is.numeric)]

sapply(col_factor, function(x){length(unique(x))})

factor_menos4 <- col_factor[sapply(col_factor, function(x){length(unique(x))<6})]
#factor_mas5 <- col_factor[sapply(col_factor, function(x){length(unique(x))>5})]

columnas <- c(names(col_numeric),)
...
# Combine the numerical and one-hot encoded data frames
combined_data <- cbind(numerical_data, encoded_data)
...
Algorimo para la detección de correlaciones entre valores perdidos y columnas para la determinación de la forma en la que
se han perdido los valores. En este caso, se estudian relaciones atendiendo al tipo de datos de las columnas, si es factoro numerica

En primer paso, se filtra por tipo de datos. Tras esto, se aplica one hot encoding para obtener columans continuas de las discretas. Dado que aplicar one hot encoding a factores con una gran cantidad de valores aumentaria demasiado la dimensionalidad, se ha decidido que solo se aplicará esta técnica a, haciendo uso de otro tipo de técnicas como el contraste de hiposesis mediante chi cuadrado para estudiar la correlacion de estas variables.
...
t <- lenght(col_numeric)
# Se extraen una columna que indique la presencia de los NA en cada columna numerica
for(i in 1:t){
  if(sum(is.na(col_numeric[i]))!=0){
    col_numeric <- col_numeric %>%
      mutate(col=is.na(col_numeric[i]))
    colnames(col_numeric)[ncol(col_numeric)] <-colnames(col_numeric)[i] +"_NA"
  }
}


# Se aplica one hot encoding con las categoricas, que tambien obtendra una columna para los NA
matriz_one_hot <- model.matrix(~ . - 1, data = factor_menos4)

# Agregar el resultado al dataframe original
df_cor <- cbind(col_numeric, matriz_one_hot)


# Con esto ya se tiene un df con una columna na para cada variable, por lo que se buscan correlaciones entre los datos

# Es necesario identificar un valor a partir de la cual la correlacion entre dos variables es significativa
# (dentro del intervalo [0,1]). En este caso se toma 0.90 para obtener las correlaciones mas fuertes y estudiarlas en
# profundidad

col_na <- grep(pattern = "_NA$",x=colnames(df_cor))
col_var <- grep(pattern = "_NA$",x=colnames(df_cor),invert = T)


# Se genera una matriz vacia que almacenara correlaciones entre NA y columnas
matriz_corr <- matrix(data = rep(0,times=length(col_na)*length(col_var)),
                      ncol = length(col_na),nrow = length(col_var))

colnames(matriz_corr) <- colnames(df_cor)[col_na]
rownames(matriz_corr) <- colnames(df_cor)[col_var]
  
# Se calcula la correlacion en cada par, salvo que procedan de la misma variable
for(col_na in (col_na)){
  for(col_var in (col_var)){
    n_na <- colnames(df_cor)[col_na]
    n_var <-colnames(df_cor)[col_var]
    n_orig <-n_na[1:(length(n_na)-3)]
    
    # Comprueba si provienen de la misma variable
    if(grepl(pattern=n_orig,x=n_var)){
     #Porviene de la misma variable 
    }
    else{
      correlacion <-cor(df_cor[[n_na]],df_cor[[n_var]],method = "spearman")
      matriz_corr[n_var,n_na] <- correlacion
    }
  }
  
}

# se visualiza por pantalla la matriz de correlaciones 

df_correlacion <- as.data.frame(as.table(matriz_corr))


# Renombrar las columnas si es necesario
names(df_correlacion) <- c("Columna", "Fila", "Valor")

# Crear un gráfico de calor con ggplot2
ggplot(df_correlacion, aes(x = Columna, y = Fila, fill = Valor)) +
  geom_tile() +
...
one_hot_encoding_with_na <- function(dataframe) {
  # Inicializar un nuevo dataframe para almacenar los resultados
  result_df <- data.frame()
  
  # Iterar sobre cada columna del dataframe original
  for (col in names(dataframe)) {
    # Realizar one-hot encoding usando la función 'model.matrix'
    one_hot_encoded <- as.data.frame(model.matrix(~ 0 + dataframe[[col]]))
    
    # Añadir una columna que indique la presencia de NA en la columna original
    one_hot_encoded$NA_indicator <- ifelse(is.na(dataframe[[col]]), 1, 0)
    
    # Renombrar las columnas para evitar conflictos de nombres
    col_names <- names(one_hot_encoded)
    new_col_names <- c(paste0(col, "_", col_names[-length(col_names)]), paste0(col, "_NA"))
    names(one_hot_encoded) <- new_col_names
    
    # Concatenar el resultado al dataframe final
    result_df <- cbind(result_df, one_hot_encoded)
  }
...
#### Se aumenta el numero de valores perdidos de forma completely at random
Para añadir mayor incertidumbre en el conjunto de datos, se ha decidido eliminar un 5% de 
los valores del dataset para complicar el análisis. El siguiente fragmento de codigo se encarga
de elimiar de nuestro dataframe estos datos

```{r}
n_datos <- ncol(df)*nrow(df)

rows <- sample(1:nrow(df), n_datos%/%5 , replace = TRUE)
cols <- sample(1:ncol(df), n_datos%/%5, replace = TRUE)

for (k in 1:length(rows)) {
  df[rows[k], cols[k]] <- NA
}

base::Map(function(i, j) {df[i, j] <- NA}, rows, cols)
...
Teniendo en cuenta la representacion grafica realizada, en este caso se imputaran 
valores perdidos por motivos evidentes en el conjunto de datos. Estos son:
*   Falta de numero de hijos cuando la columna 
