...
Una vez realizado la carga de los datos, se pasa a realizar una pequeña visualización previa antes del comienzo del análisis, para ello haciendo uso de la función `glimpse`.
...
Se tiene que el numero de instancias es de 361934 y el de atributos de 48. Además el tipo de los datos y sus primeros valores para cada atributo se observa de la siguiente forma
...
Una vez realiazda la carga de los datos, se pasa a explorar cada una de las variables de forma individual para estudiar las distribuciones de los datos y valorar la aplicación de castings no planteados en la carga.


Cabe destacar que representación por cada una de las variables se realiza atendiendo a su tipo. En este caso dado que se tienen 3 tipos de variables, numericas, strings y categóricas( representadas por un factor) el estudio se divide en tres partes

#### Variables de tipo string
```{r}
col_string <-colnames(df[sapply(df, is.character)])
print(paste("Las variables de tipo caracter son:",paste(col_string, collapse=", ")))
```
...
print(paste("Las columnas con NA de tipo factor son las siguientes"))
print(colnames(df)[col_factor_na])

print(paste("Las columnas con NA de tipo numerico son las siguientes"))
print(colnames(df)[col_numeric_na])
