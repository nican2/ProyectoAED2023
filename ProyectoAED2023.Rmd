...
Una vez realizado el tratamiento de las variables por tipos, se pasa a obtener las características generales del conjunto de datos previo a la visualización de los mismos.


Se tiene que el número de instancias es de 361934 y el de atributos de 48. Emplearemos la función *glimpse* de la librería *dplyr* para obtener una primera impresión del dataset. Mediante esta función se obtiene el tipo de los datos y los primeros valores de cada columna del dataset.
...
library("colorspace")
library('grid')
aggr(df[,1:15], prop = FALSE, combined = TRUE, numbers = TRUE, sortVars = TRUE, sortCombs = TRUE)
```
