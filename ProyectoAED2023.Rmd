...
# Para la función aggr de la sección de características del dataset
library('VIM')
library("colorspace")
library('grid')
```
...
```{r}
# Función para sacar las subcadenas en cada línea
...
# Conversión de las variables correspondientes a tipo numérico
...
# Transformación a factores de las variables pertinentes
...
### Descripción del daset

Descripción del dataset(fuente y año y demás información)


Se tiene que el número de instancias es de 361934 y el de atributos de 48. 

#### Elección de las variables de interés

Puesto que 48 son demasiadas variables, se ha decidido hacer una selección de las variables de mayor interés. De este modo, será posible obtener gráficos más claros y entendibles para el lector.

Las variables de interese seleccionadas han sido:
...
+ **CAMPO**: Campo de los estuidos.
+ **SITLAB**: Situación laboral durante la última semana.
+ **SATISTIEMP**: Grado de satisfacción en el tiempo de desplazamiento al trabajo.
+ **COMPRAINT**: Realización de compras por internet en el último mes.
+ **HIJOS**: Tenencia de hijos.
+ **NHIJOS**: Número total de hijos.
+ **TDOMEST**: Grado de participación en las tareas domésticas del hogar.

```{r include=FALSE}
df_reducido <- df[,c("IDQ_PV", "TAM_MUNI", "EC", "EDADEC", "ESTUDIOS", "ANOESTUD", "EDADESTUD", "CAMPO", "SITLAB", "SATISTIEMP", "COMPRAINT", "HIJOS", "NHIJOS", "TDOMEST")]
```

### Características generales del dataset reducido

Previamente al empleo de técnicas de visualización de los datos es importante conocer las características del dataset con el que se está tratando. Así como conocer los tipos de las variables, los diferentes valores que puede tomar cada variables, la existencia de niveles y factoes dentro de las variables o la existencia de NAs en las variables.

En primer lugar, emplearemos la función *glimpse* de la librería *dplyr* para obtener una primera impresión del dataset. Mediante esta función se obtiene el tipo de los datos y los primeros valores de cada columna del dataset.

```{r}
glimpse(df_reducido, width = 70) # El parámetro width sirve para elegir la longitud de los datos a mostrar (util para que no quede feo en el pdf)
```
...
En el caso de columnas de tipo numerico, como por ejemplo EDADEC o ANOESTUD. Se han obtenido estadísticos básicos como son el mínimo, el máximo, la mediana, la media y el primer y tercer cuartil. En cambio, en el caso de las columnas de tipo caracter, como por ejemplo IDQ_PV o TAM_MUNI. Se han obtenido las frecuencias de cada uno de los caracteres en su correspondiente columna. Además, mediante la funció *summary* también se obtiene la cantidad de NA's en cada una de las columnas.

En tercer lugar, se ha generado una tabla de gran utilidad para identificar las variables que son de tipo factor y las que tienen diferentes niveles en sus datos. Además también es posible obtener el valor del nivel más alto y su frecuencia de aparición. Por último, en la tabla también quedan representados las cantidad de NAs así como el porcentaje de los datos que representan dicha cantidad de NAs.

```{r include=FALSE}
SumarioDatos<-function(data=data,digitos=3)
{
  
  type<-sapply(data,class)
  levels<-sapply(data,function(x){length(unique(x))})
  topLevel<-sapply(data,
                   function(x){t<-table(x)
                   colnames(t(which.max(t)))})
  topCount<-sapply(data,function(x){max(table(x))})
  topFrac<-sapply(data,function(x,digitos){t<-table(x)
  (max(t)/sum(t))}) %>%round(digits=digitos)
  missFreq<-sapply(data,function(x){sum(is.na(x)==TRUE)})
  missFrac<-sapply(data,function(x){sum(is.na(x)==TRUE)/length(x)}) %>%
    round(digits=digitos)
  
  sumario<-data.frame(variable=colnames(data),type=type,levels=levels,topLevel=topLevel,
                      topCount=topCount,topFrac=topFrac,missFreq=missFreq,missFrac=missFrac)
...
```{r}
print(SumarioDatos(data=df_reducido))
```

En cuarto lugar, para finalizar con las características del dataset. Se ha generado el siguiente gráfico haciendo uso de la función *aggr* de la librería *VIM*. Mediante ella es posible representar visualmente la candidad de NAs en el dataset. Y, además permite analizar la distribución de los valores faltantes en diferentes variables y combinaciones.

```{r}
aggr(df_reducido, prop = FALSE, combined = TRUE, numbers = TRUE, sortVars = TRUE, sortCombs = TRUE)[2]
```
...
En esta sección, se eliminan valores NA y se tratan los outliers. En primer lugar, se visualiza donde se encuentran los valores faltantes

```{r}
col_na <- sapply(df, function(x) any(is.na(x)))

col_factor_na <- col_na & sapply(df, is.factor)
col_numeric_na <-col_na & sapply(df, is.numeric)
col_char_na <- col_na & sapply(df, is.character)

print(paste("El numero de columnas con NA de tipo factor es de ",sum(col_factor_na)))
print(paste("El numero de columnas con NA de tipo numerico es de ",sum(col_numeric_na)))
print(paste("El numero de columnas con NA de tipo caracter es de ",sum(col_char_na)))
