...
library(knitr)
options(width = 100)
knitr::opts_chunk$set(echo = F, message = F, error = F, warning = F, comment = NA, dpi = 100, tidy = F, cache.path = '.cache/', fig.path = './figure/', include = F, fig.width = 4.0, fig.height = 3, fig.align = "left")
```
...
aggr_result <- capture.output({
  df %>%
    select_if(~ anyNA(.)) %>%
    aggr(prop = TRUE, numbers = TRUE, sortVars = TRUE, sortCombs = TRUE, cex.axis = 0.6, cex.lab = 0.7, digits = 1, gap = 1.5, cex.numbers = 0.6, plot = TRUE)
})
