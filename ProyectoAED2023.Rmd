...
library(pacman)
pacman::p_load(readr, stringr, tidyr, dplyr, readxl, ggplot2, forcats, haven, kableExtra, VIM, e1071, ggstatsplot, gridExtra, ggmap, GGally, leaflet, leaflet.extras2, colorspace)
```
...
# Codificamos los NA de la variable NHIJOS como 0, ya que nos será útil más tarde esta interpretación. Esto se ha hecho tras asegurar que la aparición de NA es un si y solo si debido a que la variable factor HIJOS tenía un "No"
...
Así, encontramos por ejemplo que la combinación de NA en (SATISTIEMP, ANOESTUD, EDADESTUD, CAMPO) cuenta con una proporción del 26.49 % de los datos totales.
...
Una vez hemos asegurado que nuestros datos estén en la estructura de data.frame, tengan valores correctamente etiquetados y estén almacenados con el tipo correcto, podemos empezar a buscar posibles patrones en las instancias o entre las características.
...
Pasamos a hacer una visualización de estas variables.

```{r}
p_edad <- ggplot(df, aes(x = EDAD)) +
...
ggpairs útil,mirar ggcorrplot



```{r edad_by_sexo}
p_edad_sexo <- ggplot(df, aes(x = EDAD, color = SEXO)) +
  geom_freqpoly(binwidth = 1) +
  labs(
    title = "Distribución de edades en la encuesta",
    x = "Edad / años",
...
# Mapa de Jesús -- pruebas

```{r}
classify_vector <- function(vector) {
  # Calcula los cuantiles
  quantiles <- quantile(vector, probs = c(0, 0.2, 0.4, 0.6, 0.8, 1))

  # Asigna las categorías
  classified <- cut(vector, breaks = quantiles, labels = c(4, 8, 12, 16, 20), include.lowest = TRUE)
  print(classified)
  return(as.numeric(classified))
}
```
...
# Obtener las coordenadas de las capitales de provincia
...
capital_data <- merge(mean_satis, capital_locations, by = "IDQ_PV", sort = FALSE)

city_name <- "Madrid"
load("./data/center_location.RData")

# Define a color palette with more colors
n_colors <- 52  # You can adjust the number of colors as needed
...
mymap <- leaflet() %>%
  ...
  addCircleMarkers(
    ...
    label = ~IDQ_PV,
    popup = ~paste("Mean SATISTIEMP: ", round(Mean_satistiemp, 2))
  ) %>%
...
# añadir popup con media y label con nombre; addLayersControl ara controlar legenda y demas...
```
