...
Esta serie de conclusiones de pertenencia de NA no se comprueban en R.
...
```{r sitlab_bar, echo = FALSE, include = TRUE, fig.width = 4, fig.height = 4, fig.align = "center", fig.cap = "Diagrama de barras para la variable SITLAB", fig.pos = "H"}
ggplot(frecuencias, aes(x = reorder(Situacion, -n), y = n, fill = n)) +
  geom_bar(stat = "identity",
           col = "black") +
  labs(title = "Diagrama de Barras de la Situación Laboral",
       x = "Categoría",
       y = "Frecuencia") +
...
```{r mosaico, echo = FALSE, include = TRUE, fig.width = 7, fig.height = 5, fig.align = "center", fig.cap = "Gráfico de barras del campo de estuido en función del sexo", fig.pos = "H"}
library(ggmosaic)
ggplot(data = df) +
  geom_mosaic(aes(x = product(SITLAB, TDOMEST), fill = HIJOS))
#+ theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
```{r mosaico, echo = FALSE, include = TRUE, fig.width = 7, fig.height = 5, fig.align = "center", fig.cap = "Gráfico de barras del campo de estuido en función del sexo", fig.pos = "H"}
library(ggmosaic)
ggplot(data = df) +
  geom_mosaic(aes(x = product(TAM_MUNI, SITLAB), fill = HIJOS))
  + theme(axis.text.x = element_text(angle = -45, hjust=1))
```
```
